    ...
    public class DatabaseService
    {
        ...
        private T ParseResult<T>(DbDataReader reader)
        {
            ...
            foreach (var prop in props)
            {
                ...
                {
                    var sqlConverterAttr = (SqlConverterAttribute)attributes.FirstOrDefault(x => x is SqlConverterAttribute);
                    ...
                    else if (columns[ordinal].DataType.Equals(prop.PropertyType))
                        prop.SetValue(empty, dbvalue);

                    var _afterParseAttr = (AfterSqlParseAttribute)attributes.FirstOrDefault(x => x is AfterSqlParseAttribute);
                    if (_afterParseAttr?.MethodName != null)
                        destinationType.GetMethods().FirstOrDefault(m => m.Name == _afterParseAttr.MethodName && m.GetParameters().Length == 1)?.Invoke(empty, new[] { gstate });
                }
            }
            ...
            if (afterParseAttr.Length > 0)
            {
                ...
                if (attr.MethodName != null) destinationType.GetMethods().FirstOrDefault(m => m.Name == attr.MethodName && m.GetParameters().Length == (attr.SqlColumnsAsArgs?.Length ?? 0))?.Invoke(empty, attr.SqlColumnsAsArgs?.Select(cln =>
                {
        ...
        public class ContextBase
        {
            ...
            public class Guilds : ContextBase
            {
                ...
                public async Task<Model.Guild> GetGuild(ulong guildId)
                {
                    ...
                    return _service.ParseResult<Model.Guild>(await _service.GetExecuter(new Query(_tableName)
